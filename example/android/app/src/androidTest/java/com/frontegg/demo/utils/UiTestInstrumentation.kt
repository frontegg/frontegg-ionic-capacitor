...
class UiTestInstrumentation {
    ...
    /**
     * Launch the Ionic example's main Capacitor activity.
     *
     * The Kotlin demo uses a hand-written NavigationActivity; the Capacitor
     * example uses the generated com.frontegg.demo.MainActivity (extends
     * BridgeActivity). Default is wired to that.
     */
    fun openApp(
    ...
    fun clickByTextIgnoreCase(text: String, timeout: Long = timeoutMillis): Boolean {
        val selector = By.text(Pattern.compile(Pattern.quote(text), Pattern.CASE_INSENSITIVE))
        val obj = waitForView(selector, timeout = timeout)
        obj?.click()
        return obj != null
    }
